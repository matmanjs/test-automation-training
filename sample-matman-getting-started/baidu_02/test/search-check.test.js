...
describe('百度首页：搜索', function () {
  ...
  describe('第一步：开始操作之前', function () {
    ...
    it('cookie： 应该包含 mykey1=myvalue1; mykey2=myvalue2', function () {
      expect(data.cookie).to.match(
        new RegExp('mykey1=myvalue1; mykey2=myvalue2', 'gi'),
      );
    });
  ...
  describe('第三步：点击搜索按钮，获得搜索结果', function () {
    ...
    it('searchResultInfo： 搜索之后至少展示了 10 个结果', function () {
      // 之所以没有直接判断等于，是因为搜索结果可能会被插入广告导致可能有 11 个，因此采用至少10个来判断
      // expect(data.searchResultInfo.list).to.have.lengthOf(10);
      expect(data.searchResultInfo.list).to.have.lengthOf.at.least(10);
    });
  });
});
